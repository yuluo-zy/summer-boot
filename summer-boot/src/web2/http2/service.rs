...
use crate::web2::http2::error::DispatchError;
...
use super::{
    config::Service2Options, dispatcher::Dispatcher, handshake_with_timeout, HandshakeWithTimeout,
};
...
#[derive(Debug)]
pub struct Http2Server<ARW, F, Fut> {
    io: ARW,
    endpoint: F,
    opts: Service2Options,
...
impl<ARW, F, Fut> Http2Server<ARW, F, Fut> {
    /// Create new `H2Service` instance with config.
    // pub(crate) fn with_opts(io: ARW,
    //     opts: Service2Options,
    //     endpoint: F,
    // ) -> Self {
    //     Http2Server {
    ...
    fn new_service(&self, endpoint: F) -> Self::Future {
        let endpoint = self.endpoint;
        let opts = self.opts.clone();

        Box::pin(async move { Ok(H2ServiceHandler::new(cfg, on_connect_ext, service)) })
    }
}

impl<F, Fut> Http2Server<TcpStream, F, Fut>
where
    F: Fn(http_types::Request) -> Fut,
    Fut: Future<Output = http_types::Result<http_types::Request>>,
{
    ...
    T: AsyncRead + AsyncWrite + Unpin,
{
    ...
    // Established(Dispatcher<T, S, Fut, (), ()>),
    Established(),
}
