...
use crate::tcp::h2::ping;
use crate::tcp::h2::ping::Recorder;
use http_types::Body;
...
pin_project! {
    ...
    pub struct H2Stream<F>
    {
        reply: SendResponse<SendBuf<Body>>,
        #[pin]
        state: H2StreamState<F>,
    }
}

pin_project! {
    #[project = H2StreamStateProj]
    enum H2StreamState<F>
    {
        Service {
            #[pin]
            fut: F,
...
impl<F> H2Stream<F>
{
    fn new(
        fut: F,
        connect_parts: Option<ConnectParts>,
        respond: SendResponse<SendBuf<Body>>,
    ) -> H2Stream<F> {
        H2Stream {
            reply: respond,
            ...
            let next = match me.state.as_mut().project() {
                H2StreamStateProj::Service {
                    ...
                    let mut res = match h.poll(cx) {
                        Poll::Ready(Ok(r)) => r,
                        Poll::Pending => {
                            //响应还没有准备好，所以我们要检查客户端是否发送了一个 RST_STREAM 帧 本帧将取消当前请求。
                    ...
                    super::strip_connection_headers(res.header_values(), false);

                    // set Date header if it isn't already set...
                    // res.append_header(http_types::headers::DATE,);
                        // http::header::DATE)
...
impl<F, E> Future for H2Stream<F>
    where
        F: Future<Output = Result<Response, E>>,
        E: Into<Box<dyn StdError + Send + Sync>>,
{
